...
//        robot_state_ = robot_state::RobotStatePtr(new robot_state::RobotState(robot_model_));
    ...
    bool executeTrajectory(){
        ...
        std::shared_ptr<moveit::planning_interface::MoveGroup> arm_group;
        moveit::planning_interface::MoveGroup::Plan to_approach_point_plan, to_goal_plan;
        ...
        if(! goal_aquired_){
            ...
            approach_frames_vector_ = getRandomApproachFrames(number_of_frames);
        }
        ...
        while(!complete_plan && iteration < number_of_trials){
            ...
            while((vectorsDistance(goal_, vector) - kExtentionDistance) > 0.01){
                ...
                //transform approach points and final goals to the planning frame "base"
                if(!transformFrames(approach_frames_vector_, transformed_approach_frames_vector_, "approach_frame"))
                    continue;
                if(!transformFrames(goal_frames_vector_, transformed_goal_frames_vector_, "final_goal_frame"))
                    continue;
                if(transformed_approach_frames_vector_.empty() || transformed_goal_frames_vector_.empty())
                    continue;

                vector = {transformed_goal_frames_vector_[0].pose.position.x,
                            transformed_goal_frames_vector_[0].pose.position.y,
                            transformed_goal_frames_vector_[0].pose.position.z};
            ...
            // get a random index from the approach frames vector
            int random_pose_index = rand() % transformed_approach_frames_vector_.size();
            ...
            arm_group->setPoseTarget(transformed_approach_frames_vector_[random_pose_index]);
            ...
            if (to_approach_point_plan_result){
                ...
                waypoints.push_back(transformed_approach_frames_vector_[random_pose_index].pose);
                waypoints.push_back(transformed_goal_frames_vector_[random_pose_index].pose);
        ...
        goal_.clear();
        goal_frames_vector_.clear();
        transformed_approach_frames_vector_.clear();
        transformed_goal_frames_vector_.clear();
...
private:
    ...
    std::vector<geometry_msgs::PoseStamped> approach_frames_vector_;
    ...
    std::vector<geometry_msgs::PoseStamped> transformed_approach_frames_vector_, goal_frames_vector_, transformed_goal_frames_vector_;
